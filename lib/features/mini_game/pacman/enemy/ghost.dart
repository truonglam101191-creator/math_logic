...
class Ghost extends SimpleEnemy with BlockMovementCollision, PathFinding {
  ...
  Ghost({required super.position, this.type = GhostType.red})
    : super(
  ...
  void update(double dt) {
    if (enabledBeheavor) {
      seePlayer(
        ...
        radiusVision: GamePacman.tileSize * 2,
        notObserved: () {
  ...
  void _runRandom(double dt, Direction direction) {
    final canVerticalDirections = [direction, Direction.up, Direction.down];
  ...
  void bite() {
    ...
    replaceAnimation(
      SimpleDirectionAnimation(idleRight: animation, runRight: animation),
    );
  ...
  List<GameComponent> get ignoreableCollisions {
    ...
    return [if (gameRef.player != null) gameRef.player!, ...gameRef.enemies()];
  }
  ...
  void onMount() {
    ...
    // Đợi một frame để đảm bảo gameRef đã được thiết lập
    Future.microtask(() {
      if (hasGameRef) {
  ...
  void _startInitialMovement({bool withDelay = true}) async {
    if (!hasGameRef) return; // Đảm bảo gameRef đã được thiết lập
    ...
    switch (type) {
      case GhostType.red:
        ...
        moveToPositionWithPathFinding(
          Vector2(GamePacman.tileSize * 4, GamePacman.tileSize * 3),
          ignoreCollisions: ignoreableCollisions,
        );
      ...
      case GhostType.blue:
        ...
        moveToPositionWithPathFinding(
          Vector2(GamePacman.tileSize * 14, GamePacman.tileSize * 3),
          ignoreCollisions: ignoreableCollisions,
        );
        enabledBeheavor = true;
        break;
      case GhostType.pink:
        ...
        moveToPositionWithPathFinding(
          Vector2(GamePacman.tileSize * 4, GamePacman.tileSize * 13),
          ignoreCollisions: ignoreableCollisions,
        );
        enabledBeheavor = true;
        break;
      case GhostType.orange:
        ...
        moveToPositionWithPathFinding(
          Vector2(GamePacman.tileSize * 14, GamePacman.tileSize * 13),
          ignoreCollisions: ignoreableCollisions,
        );
        enabledBeheavor = true;
        break;
  ...
  @override
  Future<void> onLoad() {
    add(RectangleHitbox(size: size - Vector2.all(4), position: Vector2.all(2)));
    return super.onLoad();
  }
