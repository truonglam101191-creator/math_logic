...
mixin CustomMovementByJoystick on SimplePlayer {
  ...
  void onJoystickChangeDirectional(JoystickDirectionalEvent event) {
    ...
    if (event.directional != JoystickMoveDirectional.IDLE &&
      ...
      final dots = gameRef.collisions().where((element) {
  ...
  void _correctPositionToCenterTile({bool x = true, bool y = true}) {
    int w = (position.x / GamePacman.tileSize).round();
    int h = (position.y / GamePacman.tileSize).round();
    position = position.copyWith(
      x: x ? w * GamePacman.tileSize : null,
      y: y ? h * GamePacman.tileSize : null,
    );
  }
