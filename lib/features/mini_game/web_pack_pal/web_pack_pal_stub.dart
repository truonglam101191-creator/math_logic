...
class WebPackPalStub extends StatefulWidget {
  const WebPackPalStub({super.key, this.onGameEnd});

  final void Function(int score)? onGameEnd;

  @override
  State<WebPackPalStub> createState() => _WebPackPalStubState();
}

class _WebPackPalStubState extends State<WebPackPalStub> {
  static const String _kSavedGameStateKey = 'pack_pal_saved_state_v1';
    ...
    try {
      ...
      try {
        String html = await rootBundle.loadString(
          'assets/pack_pal/pack_pal.html',
        );

        final config = await rootBundle.loadString(
          'assets/pack_pal/config.json',
        );

        if (config.isNotEmpty) {
          html = html.replaceAll(
            '<script id="gameConfigPackPalettes"></script>',
            '<script id="gameConfigPackPalettes"> window.gameConfig = $config ;</script>',
          );
        }

        final assets = await rootBundle.loadString(
          'assets/pack_pal/assetMap.json',
        );
  ...
  Future<void> _handleJsMessage(dynamic payload) async {
    try {
      ...
      // --- handle explicit game_end (successful completion) ---
      if (parsed.isNotEmpty && parsed['type'] == 'game_end') {
        final s = parsed['score'];
        final int? score = s is int ? s : int.tryParse(s?.toString() ?? '');

        // prefer an explicit gameState/payload sent by the web game
        dynamic stateObj = (parsed['gameState'] ?? parsed['payload'] ?? parsed);
        ...
        try {
          ...
          bool saved = false;
          if (stateObj is Map) {
            // common level keys to look for
            const levelKeys = ['level', 'currentLevel', 'levelIndex'];
            for (final k in levelKeys) {
              if (stateObj.containsKey(k)) {
                prefs.setString(_kSavedGameStateKey, jsonEncode(stateObj));
                saved = true;
                break;
              }
          ...
          if (!saved) {
            // attempt to load existing persisted state and advance level
            final raw = prefs.getString(_kSavedGameStateKey);
            if (raw != null && raw.isNotEmpty) {
              try {
                final decoded = jsonDecode(raw);
                if (decoded is Map<String, dynamic>) {
                  // try to increment any numeric level-like key
                  const levelKeys = ['level', 'currentLevel', 'levelIndex'];
                  ...
                  if (!incremented) {
                    // nothing to increment; remove persisted state to fallback to defaults
                    await prefs.remove(_kSavedGameStateKey);
                  }
                }
              } catch (_) {
                await prefs.remove(_kSavedGameStateKey);
          ...
          debugPrint('Failed updating saved state on game_end: $e');
        }
