...
class BoardManager extends StateNotifier<Board> {
  ...
  void load() async {
    ...
    final data = await serviceLocator
        .get<DataBaseFuntion>()
        .getGem2048HighScore();
    state = data != null ? Board.fromJson(data) : _newGame();
  }
  ...
  void save() async {
    ...
    // final  serviceLocator.get<DataBaseFuntion>().getGem2048HighScore();

    //var box = await Hive.openBox<Board>('boardBox');
    try {
      serviceLocator.get<DataBaseFuntion>().saveGem2048HighScore(
        value: state.toJson(),
      );
    } catch (e) {
      print(e);
