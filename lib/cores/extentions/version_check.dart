...
typedef GetStoreVersionAndUrl =
    Future<StoreVersionAndUrl?> Function(String packageName);
typedef ShowUpdateDialog =
    void Function(BuildContext context, VersionCheck versionCheck);
...
Future<StoreVersionAndUrl?> _getAndroidStoreVersionAndUrl(
  ...
  if (resp.statusCode == 200) {
    ...
    try {
      ...
      final cv = elements.firstWhere(
        (element) => element.text == 'Current Version',
      );
...
StoreVersionAndUrl? redesignedVersion(dom.Document response, String url) {
  try {
    ...
    final versionElement =
        additionalInfoElementsFiltered
